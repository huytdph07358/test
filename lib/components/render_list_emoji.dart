...
class _ListEmojiWidgetState extends State<ListEmojiWidget> {
  ...
  caculatorHeightItem(category) {
    ...
    if (index == -1) return 0;
    double row = listCategories[index]["num"] / 7;
  ...
  caculatorScrollTo(categoryName){
    ...
    for(int i = 0; i < index; i++){
      result += caculatorHeightItem(listCategories[i]["name"]) * 56 + 16; 
    }
    return result;
  ...
  Widget renderListCategory() {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return listCategories[index]['name'] == 'recent' ? Container() : InkWell(
          onTap: () {
            setState(() {
              select = e["name"];
            });
  ...
  pickImage() async {
    try {
      ...
      image = {
        ...
        "bytes": isGif ? myMultipleFiles[0]["file"] :  Img.encodePng(thumbnail)
      };
      setState(() {});
    } catch (e) {
      print(" pickImage $e");
