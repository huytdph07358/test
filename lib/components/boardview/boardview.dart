...
class BoardViewState extends State<BoardView> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    var newList = widget.lists!.toList();

    Widget list2 = PageView.builder(
      itemCount: 1,
      itemBuilder: ((context, index) {
        return ScrollSnapList(
          listController: boardViewController,
          itemBuilder: (BuildContext context, int index) {
            if (newList[index].boardView == null) {
              newList[index] = BoardList(
                items: newList[index].items,
                backgroundColor: newList[index].backgroundColor,
                footer: newList[index].footer,
                header: newList[index].header,
                boardView: this,
                draggable: newList[index].draggable,
                onDropList: newList[index].onDropList,
                onTapList: newList[index].onTapList,
                onStartDragList: newList[index].onStartDragList,
              );
            }
            if (newList[index].index != index) {
              newList[index] = BoardList(
                items: newList[index].items,
                backgroundColor: newList[index].backgroundColor,
                footer: newList[index].footer,
            ...
            var temp = Container(
              ...
              child: Row(
                ...
                children: <Widget>[Expanded(child: newList[index])],
              ));
          ...
          if (dy! > tempBottom! - 70) {
            ...
            if (listStates[draggedListIndex!].boardListController.hasClients) {
              ...
              listStates[draggedListIndex!].boardListController.animateTo(
                ...
                if (draggedListIndex != null && listStates[draggedListIndex].boardListController.hasClients) pos -= listStates[draggedListIndex].boardListController.position.pixels;
