...
class _InviteChannelState extends State<InviteChannel> {
  ...
  _invite(token, workspaceId, channelId , user) {
    ...
    Provider.of<Channels>(context, listen: false).inviteToChannel(token, workspaceId, channelId, email, 1, user);
    onInviteToChannel(token, workspaceId, channelId, search);
  }
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                    ...
                    trailing: Container(
                      ...
                      child: TextButton(
                        child: Text(
                          members[index]["invite"] == "Invite" ? "Invite" : "Invited",
                          style: TextStyle(
                            fontSize: 13,
                            color: (validate(members[index]["id"]) == true && members[index]["invite"] == "Invite")
                                ? Utils.getPrimaryColor()
                                : Colors.grey)
                            ),
                        onPressed: (validate(members[index]["id"]) == true && members[index]["invite"] == "Invite")
