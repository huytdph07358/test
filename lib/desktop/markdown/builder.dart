...
import '../../components/attachments/text_file.dart';
...
class MarkdownBuilder implements md.NodeVisitor {
      ...
      if (_isListTag(tag)) {
        ...
        if (_listIndents.isNotEmpty) {
          ...
          if (el is md.Element && el.attributes['type'] == 'checkbox') {
            bool val = (el.attributes['checked']).toString().toLowerCase() == 'true';
        ...
        child = SizedBox( 
          ...
          child:element.textContent=="" ? SizedBox(): CustomHighlightView( 
            ...
            textStyle: GoogleFonts.robotoMono(color: Color.fromARGB(255, 145, 183, 212)), 
          ), 
        );
      } else if (tag == 'hr') {
  ...
  onShowIssueInfo(issue) {
    ...
    var workspaceId = issue["workspace_id"];
    var channelId = issue["channel_id"];
  ...
  Widget _buildRichText(TextSpan? text, {TextAlign? textAlign}) {
    var newText = text == null ? text : (text.text ?? "").contains(RegExp(r'={7}[@|#]')) ? parseMention(text) : text;
    if (selectable) {
